...
@SpringBootTest
public class RatingControllerITest extends AllTestITest {
    ...
    class Validate {
        ...
        void validate_withAdmin() throws Exception {
            ...
            mockMvc.perform(post("/rating/validate").with(httpBasic("admin","admin")).flashAttr("rating",rating))
        ...
        void validate_withUser() throws Exception {
            Rating rating = new Rating("test","test","test",1);
            mockMvc.perform(post("/rating/validate").with(httpBasic("user","user")).flashAttr("rating",rating))
                    .andExpect(status().is3xxRedirection());
        }

        @Test
        void validate_withInvalidField() throws Exception {
            Rating rating = new Rating(null,"test","test",1);
            mockMvc.perform(post("/rating/validate").with(httpBasic("user","user")).flashAttr("rating",rating))
                    .andExpect(status().isOk())
    ...
    class Update {
        ...
        void update_withAdmin() throws Exception {
            Rating rating = new Rating("test","test","test",1);
            mockMvc.perform(post("/rating/update/1").with(httpBasic("admin","admin")).flashAttr("rating",rating))
                    .andExpect(status().is3xxRedirection());
        }
        ...
        void update_withUser() throws Exception {
            Rating rating = new Rating("test","test","test",1);
            mockMvc.perform(post("/rating/update/1").with(httpBasic("user","user")).flashAttr("rating",rating))
                    .andExpect(status().is3xxRedirection());
        }
